...
import CustomTextArea from 'components/elements/form/CustomTextArea';
...
import BridgeRight from './BridgeRight';

const BridgeInformation = ({
   TabConsumer, course, isLoading, createBridge, isCommunity, site,
}) => {
   const desc = "Unlock the world of product design with our comprehensive 'Product Design Fundamentals' online course. Whether you're an aspiring designer or a professional looking to sharpen your skills, this course provides the essential foundation you need to succeed in the field of product design. By the end of this course, you'll have a solid understanding of the fundamental principles of product design and a portfolio of work that demonstrates your skills and creativity. Join us and start your journey towards becoming a successful product designer!";
  
   const [bridge, setBridge] = useState({
      ...course.bridge_page, 
      is_video: course?.bridge_page?.is_video === undefined ? 0 : course?.bridge_page?.is_video,
      title: course.bridge_page?.title || course.name, 
      enableBridge: isCommunity ? course.bridge_page?.enableBridge !== false : true,
      image: isCommunity ? course.bridge_page?.image || bannerImg : course.bridge_page?.image,
      image_right: isCommunity ? course.bridge_page?.image_right || communityCover : course.bridge_page?.image_right,
   } || {
      is_video: 0,
      ...
      enableBridge: isCommunity ? course.bridge_page?.enableBridge !== false : true,
      image: isCommunity ? course.bridge_page?.image || bannerImg : course.bridge_page?.image,
      image_right: isCommunity ? course.bridge_page?.image_right || communityCover : course.image_right,
   });
   
   const handleSaveBridge = () => {
      createBridge({ id: course.id, inputs: bridge });
   };

   const handleChangeBridge = (key, value, file) => {
      if (key === 'imgVid') {
         if (file?.type.includes('video')) {
            setBridge({ ...bridge, video: value, is_video: 1 });
         } else {
            setBridge({ ...bridge, image: value, is_video: 0 });
         }
      } else if (key === 'imgVid_right') {
         if (file?.type.includes('video')) {
            setBridge({ ...bridge, video_right: value, is_video_right: 1 });
         } else {
            setBridge({ ...bridge, image_right: value, is_video_right: 0 });
         }
      } else {
         setBridge({ ...bridge, [key]: value });
      }
   };
   ...
   const previewBridge = () => {
      let win;
      if (course.type === '0') {
         win = window.open(`/bridge/${ course.id }?isPreview=true`, '_blank');
      } else if (course.type === '1') {
         win = window.open(`/bridge/${ course.id }?video=true&isPreview=true`, '_blank');
   ...
   const handleOpenLink = () => {
      window.open('https://support.miestro.com/716814-Bridge-Page', '_blank');
   };

   const handleRemoveBranding = () => {
      if (course.type === '1') {
         return site.membership.active_school_room.remove_branding;
      } 
      return site.active_school_room.remove_branding;
   };

   const handleBannerImg = () => {
      return bridge.is_video ? bridge.video : bridge.image;
   };

   const handleThumbnailImg = () => {
      return bridge.is_video_right ? bridge.video_right : bridge.image_right;
   };

   // Check if dark mode using school room settings
   const isDarkMode = () => {
      if (course.type === '1') {
         return site.membership.active_school_room.mode === 1;
      } 
      return site.active_school_room.mode === 1;
   };

   // Render the appropriate logo based on dark mode
   const renderLogo = () => {
      if (isDarkMode()) {
         // SVG for dark mode
         return (
   ...
   return (
      ...
      <div className={ isCommunity ? 'course__bridge course__bridge__community scroll' : 'course__bridge' }>
         <div
            style={ {
               background: '#fff',
            } }
            ...
            <Icon name='ToolTipI' />
            <Text 
               inner='Learn More About Connect Page'
               size={ sizes.size_14 }
         ...
         <AdminContainer.Header>
            <HeaderTypeFirst
               title='Connect Page'
               goBack={ isCommunity ? null
               //  () => {  history.push(`/admin/community/${ course.communityId }`);} 
                  : () => TabConsumer.switchTab('program-general') }
               isBridge={ true }
               handleSaveAndContinue={ handleSaveBridge }
               handleChange={ handleChangeBridge }
               bridge={ bridge }
               previewPlaylist={ previewBridge }
         ...
         {(isLoading) ? (
            ...
            <AdminContainer.Content>
               <div className='course__bridge__content'>
                  <div className='course__bridge__content__left'>
                     <CustomTextArea
                        placeholder='Write a title'
                        onInputChange={ (key, value) => handleChangeBridge(key, value) }
                        title={ bridge.title }
                        name='title'
                        style={ {
                           background: 'inherit',
                     ...
                     <UploadImage
                        ...
                        recomenededText='1920x1080'
                        // recomendation='1920x1080'
                        // recomendationLeft={ true }
                        recomendedTextTop={ true }
                        btnText='Image Or Video'
                        onChange={ handleChangeBridge }
                        fileLessonFormat='media'
                        fileType={ bridge.is_video ? 'video' : 'image' }
                        isImageUpload={ !!bridge.is_video }
                     />
                     {/* <CustomTextArea
                     ...
                     {/* <TextArea
                        ...
                        onChange={ (key, value) => handleChangeBridge(key, value) }
                        value={ bridge.description }
                        name='description'
                        style={ { height: '250px', borderColor: '#E7E9E9', fontSize: '16px' } }
                        maxLength={ 2000 }
                        hasHelpText={ true }
                     /> */}
                     <QuillEditor
                        ...
                        placeholder='Add your description here'
                        onChange={ (value) => handleChangeBridge('description', value) }
                        title=''
                        withoutBorder={ true }
                        removeFontSize={ true }
                     />
                     {course.type === '0' && (
                        <div className='course__bridge__content__left__lessons'>                      
                           {course.sections.map((e, i) => {
                              return (
                                 <Lessons
                                    key={ e.id }
                                    number={ i + 1 }
                  ...
                  <div className='course__bridge__content__right'>
                     <div className='course__bridge__content__right__top'> 
                        <div className='course__bridge__content__right__header'>
                           <LinkViewWithEditOneLine
                              label='Connect Page Link'
                              copyUrl={ course.type === '1' ? `${ window.location.origin }/bridge/${ course.id }?video=true` : `${ window.location.origin }/bridge/${ course.id }` }
                              isValid={ true }
                              constantUrlStart={ `${ window.location.origin }/bridge/` }
                              editableLink={ course.type === '1' ? `${ course.id }?video=true` : course.id }
                              constantUrlEnd=''
                              disableEdit={ true }
                           />
                           <UploadImage
                              label='Connect Page Thumbnail'
                              name='imgVid_right'
                              isOptional={ false }
                              src={ handleThumbnailImg() }
                              otherProps={ {
                                 cropRatio: '864x486',
                              } }
                              size='full'
                              isHaveRecomenededText={ true }
                              recomenededText='864x486'
                              recomendedTextTop={ true }
                              btnText='Image'
                              onChange={ handleChangeBridge }
                              fileLessonFormat='image'
                              fileType={ bridge.is_video_right ? 'video' : 'image' }
                              isImageUpload={ !!bridge.is_video_right }
                           />
                           <CustomTextArea
                              placeholder='Write a title'
                              onInputChange={ (key, value) => handleChangeBridge(key, value) }
                              title={ bridge.title_right || course.name }
                              name='title_right'
                              style={ {
                                 fontSize: '24px',
                                 fontWeight: '700',
                           ...
                           <TextArea
                              ...
                              onChange={ (key, value) => handleChangeBridge(key, value) }
                              value={ bridge.description_right }
                              name='description_right'
                              style={ { height: '80px', borderColor: '#E7E9E9', fontSize: '16px' } }
                              maxLength={ 200 }
                              hasHelpText={ true }
                           />
                        </div>
                    
                        <BridgeRight course={ course } isAdmin={ true } isColorLight={ true } />
                     </div>
...
export default BridgeInformation;