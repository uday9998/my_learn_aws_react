...
import BridgeRight from 'views/pages/DesignCourse/BridgeInformation/BridgeRight';
import Icon from 'components/elements/Icon';
import QueryParams from 'utils/QueryParams';
import TextWithSeeMore from 'components/modules/TextWithSeeMore';
import Text, { TYPES as types, SIZES as sizes } from 'components/elements/TextNew';
import Signup from 'containers/pages/guest/auth/SignupStudentContainer';
...
import communityCover from 'assets/images/community/cover.png';
import { SafeHtml } from 'utils/sanitizeHtml';
import LessonPageSection from './LessonPageSection';
import VideoOrImg from './VideoOrImg';

const OfferLessonsPage = ({ textColor: propTextColor }) => {
   const {
      goBackToCourses, selectedOfferCourse, template, schoolRoomSettings, user, 
      match, type, uuid, handleAttachMemberToFreeOffer, site,
   } = React.useContext(OfferContext);
   const [isOpenSignup, setIsOpenSignup] = useState(false);
   const breadcrumbLinks = [
      { text: 'Back', goTo: () => goBackToCourses() },
   ];
   const content = template[5];
   const contentItem = content.school_room_components[0];

   const getSchoolRoomSettings = () => {
      if (selectedOfferCourse.type === '1') {
         return site.membership.active_school_room;
      }
      return site.active_school_room;
   };

   const isDarkMode = () => {
      return getSchoolRoomSettings().mode === 1;
   };

   const getBackgroundColor = () => {
      if (selectedOfferCourse.type === '1') {
         return site.membership.landing_data[0].school_room_section.props.bgColor 
            || site.membership.active_school_room.school_bg_color;
      }
      return template[0].school_room_section.props.bgColor 
         || site.active_school_room.school_bg_color;
   };
   ...
   const getTextColor = () => {
      ...
      return isDarkMode() ? '#fff' : '#131F1E';
   };

   const getRemoveBranding = () => {
      return getSchoolRoomSettings().remove_branding;
   };

   const onClickJoin = () => {
      if (selectedOfferCourse.pricings.length === 1 && selectedOfferCourse.pricings?.[0].pricing_type === 0 && !user) {
         const courseId = selectedOfferCourse.plan_for_course[0].id;
         localStorage.setItem('courseId', courseId);
         localStorage.setItem('isOnlineCourse', !Number(selectedOfferCourse.type) ? 'onlinecourse' : Number(selectedOfferCourse.type) === 1 ? 'membership' : 'community');
         if (selectedOfferCourse.type === '2') {
      ...
      if (selectedOfferCourse.type === '2' && selectedOfferCourse.pricings?.[0]) {
         if (selectedOfferCourse.pricings.length === 1 && selectedOfferCourse.pricings?.[0].pricing_type === 0) {
            if (user && user.id) {
               handleAttachMemberToFreeOffer(selectedOfferCourse.pricings?.[0].plan_id, selectedOfferCourse);
            } else {
               window.open('/register', '_blank');
            }
            return;
         }
         let url = `${ process.env.REACT_APP_CHECKOUT_URL }${ uuid }/0/${ selectedOfferCourse.pricings?.[0].plan_id }`;
         if (user && user.id) {
            url = `${ process.env.REACT_APP_CHECKOUT_URL }${ uuid }/${ user.id }/${ selectedOfferCourse.pricings?.[0].plan_id }`;
         }
         window.open(url, '_blank');
      } else if (selectedOfferCourse.type === '0') {
         window.open(`/programs/${ selectedOfferCourse.url }`, '_blank');
      } else if (selectedOfferCourse.pricings?.[0] && selectedOfferCourse.type === '1') {
         let url = `/programs/${ selectedOfferCourse.url }`;
    
         if (QueryParams.get('category')) {
            url += `/${ QueryParams.get('category') }`;
            if (QueryParams.get('video')) {
               url += `?video=${ QueryParams.get('video') }`;
            }
         } else if (QueryParams.get('playlist')) {
            url += `/playlists/${ QueryParams.get('playlist') }`;
            if (QueryParams.get('video')) {
               url += `?video=${ QueryParams.get('video') }`;
            }
         } else {
            url += `/${ selectedOfferCourse.lessons[0]?.categories[0]?.link }`;
            url += `?video=${ selectedOfferCourse.lessons[0]?.id }`;
         }
   ...
   const checkVideoOrImgExist = (image, video, isVideo) => {
      if (image && !isVideo) {
         return true;
      }
      if (video && isVideo) {
   ...
   const handleBannerImg = () => {
      if (selectedOfferCourse.type === '2') {
         return bridge.image || bannerImg;
      } 
      return bridge.image;
   };

   const handleThumbnailImg = () => {
      if (selectedOfferCourse.type === '2') {
         return bridge.image_right || communityCover;
      } 
      return bridge.image_right;
   };
   ...
   const bridge = window.bridge || selectedOfferCourse.bridge_page || {};
   const isCommunity = selectedOfferCourse.type === '2';
   const isMembership = selectedOfferCourse.type === '1';
   const bgColor = getBackgroundColor();
   const textColor = getTextColor();

   return (
      <div
         className={ `lesson__page ${ schoolRoomSettings.school_room_theme_name }` }
         style={ isCommunity ? { background: '#F4F7F7' } : { background: bgColor } }
      >
         <div className='lesson__page__container'>
            { !QueryParams.get('isPreview') && (
               <div className='bread_left_icon'>
                  <Icon onClick={ handleNavigate } name='ArrowLeft' color={ textColor } />
                  <BreadCrumb links={ breadcrumbLinks } linkColor={ textColor } /> 
               </div>
            )}
            <div
               className='lesson__page__container__title'
               style={ { marginBottom: '32px' } }
            />
            <div className='lesson__page__content'>
               <div
                  className='lesson__page__content__sections' 
               > 
                  <div
                     className='lesson__page__content__sections__top'
                     style={ { background: isCommunity ? '#fff' : isMembership ? 'rgba(255, 255, 255, 0.05)' : 'var(--mainBg005)' } }>
                     <SafeHtml
                        className='lesson__page__content__sections__title'
                        html={ bridge.title || selectedOfferCourse.name }
                        style={{ color: textColor }}
                     />
                     {bridge
                   && checkVideoOrImgExist(handleBannerImg(), bridge.video, bridge.is_video)
                    && (

                       <VideoOrImg
                          image={ handleBannerImg() }
                          video={ bridge.video }
                          isVideo={ bridge.is_video } />
                    )}
                     {(bridge.description || selectedOfferCourse.description) && (
                        <SafeHtml
                           className='lesson__page__content__sections__desc'
                           html={ bridge.description || selectedOfferCourse.description }
                           style={{ color: textColor }}
                        />
                     )}
                  </div>
                  {selectedOfferCourse.type !== '1' && (
                     <div className='lesson__page__content__sections__lessons'>
                        {selectedOfferCourse.sections.map((e, index) => {
                           return (
                              <LessonPageSection
                                 key={ e.id }
                                 number={ index + 1 }
                                 course={ selectedOfferCourse }
                                 textColor={ propTextColor }
                                 section={ e }
                                 user={ user }
                                 templateCardBackground={ contentItem.props.bgColor }
                                 isColorLight={ !isDarkMode() }
                              />
                           );
                        })}
                     </div>
               ...
               <div className='lesson__page__content__right__general'>
                  <div
                     className='lesson__page__content__right'
                     style={ { background: isCommunity ? '#fff' : isMembership ? 'rgba(255, 255, 255, 0.05)' : 'var(--mainBg005)' } }>
                     <div className='lesson__page__content__right__header'>
                        {bridge 
                     && checkVideoOrImgExist(handleThumbnailImg(), bridge.video_right, bridge.is_video_right)
                      && (
                         <VideoOrImg 
                            image={ handleThumbnailImg() }
                            video={ bridge.video_right }
                            isVideo={ bridge.is_video_right } 
                            isSmall={ true }
                         />
                      )}
                        <SafeHtml
                           className='lesson__page__content__right__title'
                           html={ bridge.title_right || selectedOfferCourse.name }
                           style={{ color: textColor }}
                        />
                        {selectedOfferCourse.type === '2' && (
                           <LinkViewWithEditOneLine
                              label=''
                              copyUrl={ `${ window.location.origin }/portal/community/${ selectedOfferCourse.community_id }` }
                              isValid={ true }
                              constantUrlStart={ `${ window.location.origin }/portal/community/` }
                              editableLink={ selectedOfferCourse.community_id }
                              constantUrlEnd=''
                              disableEdit={ true }
                           />
                        )}
                        {bridge.description_right && (
                           <TextWithSeeMore
                              text={ bridge.description_right }
                              maxLength={ 100 } 
                              isBridge={ true }
                              style={{ color: textColor }} />
                        )}
                  ...
                  {!getRemoveBranding() && (
                     <div className='poweredByBridge' onClick={ (() => window.open('https://miestro.com', '_blank')) } role='presentation'>
                        <Text
                           inner='Powered By'
         ...
         {isOpenSignup && (
            <ModalNew
               ...
               className='communitySignupModal'
            >
               <Signup />
            </ModalNew>
         )}
...
OfferLessonsPage.propTypes = {
   textColor: PropTypes.string,
};

export default OfferLessonsPage;