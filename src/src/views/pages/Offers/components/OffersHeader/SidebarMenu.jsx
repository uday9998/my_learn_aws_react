...
const SidebarMenu = ({ user, logout, setIsOpenMenu, portalMenuData = {} }) => {
  ...
  useEffect(() => {
    // Check if the current URL contains "onlinecourse"
    setIsOnlineCourse(window.location.pathname.includes('onlinecourse'));
  }, []);
  ...
  const handleMainPortal = () => {
    if (window.location.pathname.includes('onlinecourse')) {
      window.location = '/portal/onlinecourse/my-portals';
    } else {
  ...
  return (
    <div className="sidebar-style-menu">
      ...
      <div className="nav-section">
        ...
        {/* Only render My Library if not in onlinecourse */}
        {!isOnlineCourse && (
        ...
        {/* Only render Community if not in onlinecourse */}
        {!isOnlineCourse && (
          <div 
            className="offers__header__right__popover__item"
