...
const OffersFooter = () => {
   const { template, isEditor, onClickElement, schoolRoomSettings, site } =
      React.useContext(OfferContext);
   const { t } = useUITranslation();

   const footerIndex = 7;
   const footer =
      template && template.length > footerIndex ? template[footerIndex] : null;
   ...
   const getFooterCustomLinksSort = () => {
      if (!site?.custom_links?.items) return [];

      return site.custom_links.items
         .filter(
            (link) => 
               link?.position === "f_right" || link?.position === "f_left"
         )
   ...
   const privacyPolicy = components.find(
      (comp) => comp?.name === "Privacy Policy"
   );
   ...
   const copyrightText = (() => {
      const defaultText = `${t('copyright')} © ${getCurrentYear()}`;
      const customText = copyright?.props?.text;

      if (!customText) return defaultText;

      // Check if it's a default copyright pattern and translate "Copyright"
      ...
      if (yearRegex.test(customText)) {
         return customText.replace(
            ...
            yearRegex,
            `${t('copyright')} © ${getCurrentYear()}`
         );
      }

      // If it starts with "Copyright", translate that word
      if (customText.toLowerCase().startsWith('copyright')) {
         return customText.replace(/^Copyright/i, t('copyright'));
      }

      return customText;
   })();

   const copyrightColor = copyright?.props?.color || "#FFFFFFB2";
   const copyrightFontSize = copyright?.props?.fontSize || "16";
   const isPrivacyVisible = privacyPolicy?.props?.visibility !== false;
   const isTermsVisible = termsComponent?.props?.visibility !== false;
   const isCopyrightVisible = copyright?.props?.visibility !== false;

   const footerBgColor = footerSection?.props?.bgColor || "#27282B";


   // Simple dark mode detection based on schoolRoomSettings.mode
   const isDarkFooter = schoolRoomSettings?.mode === 1;

   const showBranding = schoolRoomSettings?.remove_branding !== 1;

   const renderFooterContent = () => {
      const customLinks = getFooterCustomLinksSort();

      return (
         <div className="offers__footer__content">
            <div className="offers__footer__top">
               <div className="offers__footer__left">
                  {customLinks.length > 0 && (
                     <div className="custom-links footer-links">
                  ...
                  {isPrivacyVisible && (
                     <span
                        role="presentation"
                        data-action="onClickPrivacy"
                        style={{
                           fontSize: `${
                              privacyPolicy?.props?.fontSize || 16
                           }px`,
                        }}
                  ...
                  {isTermsVisible && (
                     <span
                        role="presentation"
                        data-action="onClickTerms"
                        style={{
                           fontSize: `${
                              termsComponent?.props?.fontSize || 16
                           }px`,
                        }}
               ...
               {isCopyrightVisible && (
                  <div
                     className="offers__footer__copyright"
                     style={{
               ...
               {showBranding && (
                  <div className="offers__footer__right">
                     <span className="offers__footer__right__branding">
                        Powered By
                     </span>
