...
const AdminSidebar = ({
   ...
   const {
      ...
      gbDefaultSize, gbSize, progresPercent, isBarSmall, app, mainApp, metas, openAIAssistantModal
   } = sidebarProps;
   ...
   const handleShowPopup = (title) => {
      if (!Array.isArray(permissions)) {
         if (permissions.affiliate_program && title === 'affiliate' && 
            ...
            !authUser.plan_name.includes('starter')) {
            goTo(Router.route('ADMIN_AI_ASSISTANT').getCompiledPath());
         } else {
            setPopupTitle(title);
            setShowPopup(true);
         }
