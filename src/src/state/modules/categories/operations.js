import {
   ...
   updateCategory,
   removeCategory,
   getDettachedCategoryCourses,
   attachCoursesCategory,
   detachCoursesCategory,
} from 'api/AuthApi';
import * as actions from 'state/modules/categories/actions';
...
export const reorederCategoriesOperation = (categoryIds) => {
   return async (dispatch) => {
      try {
         ...
         await reorderCategories({ category_ids: categoryIds });
      } catch (error) {
...
export const updateCategoryOperation = (inputs, id) => {
   return async dispatch => {
      dispatch(actions.updateCategoryStart());
      try {
         await updateCategory(id, inputs);
...
export const getDetachedCoursesOperation = (id) => {
   return async dispatch => {
      ...
      try {
         const { data } = await getDettachedCategoryCourses(id);
...
export const attachCourseCategoryOperation = (categoryId, ids, isCreatePage) => {
   return async dispatch => {
      ...
      try {
         const { data } = await attachCoursesCategory(categoryId, ids);
         if (isCreatePage) {
            dispatch(push({
...
export const detachCoursesCategoryOperation = (categoryId, ids) => {
   return async dispatch => {
      dispatch(actions.attachDetachCourseStart());
      try {
         const { data } = await detachCoursesCategory(categoryId, ids);
         dispatch(getCategoriesOperation());
         if (isPrint('Changes saved successfully.')) {
            toast.success('Changes saved successfully.');
         }
