...
import CreateCourseContainer from 'containers/pages/admin/designCourse/courses/createPricing';
...
const PlanEdit = ({
   ...
   goToUpsellPreview, goToUpsellEdit, goToDownsellEdit, goToDownsellPreview, type, authoresponderListOptionsInProgress,
}) => {
   ...
   return (
      ...
      <>
         {
            showPopup && createPortal(<PricingPopup popupTitle={ popupTitle } handleClosePopup={ handleClosePopup } isCheckout={ popupTitle === 'checkout' } />, document.body)
         }
         {match.path && match.path.includes('create') && <CreateCourseContainer id={ match.params.id } type={ type } communityId={ match.params.communityId } location={ location } />}
         {match.path && !match.path.includes('create') && (
            <>
               <MobileHeader>
                  <SiteHeader
                     isLeftAction
                        ...
                        <div className='plan__content'>
                           ...
                           !isMobile && (
                              ...
                              <HeaderPLan
                                 ...
                                 planName={ match && match.url.includes('membership') ? 'Membership' : plan.name }
                                 courses={ plan.courses }
                                 isHaveCoursesToDelete={ plan.after_delete_courses_unpublished }
                                 goBack={ type === 'community' ? null : () => goBack() }
                                 onSave={ () => {
                                    handleSave();
                                 } }
                                 isMobile={ isMobile }
                              />
                              <Tabs
                                 variants={ (match && match.url.includes('membership')) ? tabVariantsMembership : tabVariants }
...
const mapDispatchToProps = (dispatch) => {
   return {
      ...
      handleAddTag: (inputs) => {
         dispatch(operations.planAddTagOperation(inputs));
      },
