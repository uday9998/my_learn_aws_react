...
import {
   getDownsell,
   getPricingsForUpsell, downsellUpdate,
} from 'api';
...
const UpsellCreate = ({ match }) => {
   ...
   const [update] = useSubmitForm(downsellUpdate);
   ...
   const [getDownsellInputs, { loading: upsellLoading }] = useSubmitForm(getDownsell);
   ...
   useEffect(() => {
      initOffers([match.params.offerId], (data) => {
         ...
         getDownsellInputs(match.params.downsellId, (response) => {
            onChange({
               ...response,
            ...
            const pricingsArray = findedOffer.map((e) => ({ label: `$${ e.price ? e.price : '0.00' }`, value: e.id }));
   ...
   const generateEditingTemplate = (sections) => {
      return sections.map((e) => {
         if (e.upsell_section) {
            return {
               ...e,
               upsell_component: e.upsell_components.sort((s, a) => s.props.order - a.props.order),
            };
         }
         return {
            upsell_section: {
            ...
            upsell_components: e.downsell_components.sort((s, a) => s.props.order - a.props.order),
         };
      });
   };
   const handleInputChange = (name, value) => {
      ...
      if (name === 'headline' || name === 'description') {
         const newSections = generateEditingTemplate(inputs.checkout_landings[0].downsell_section);
   ...
   const saveDownsell = () => {
      ...
      update({
         ...
         downsellId: match.params.downsellId,
         upsellId: inputs.upsell_id,
         data: {
            ...
            sections: generateEditingTemplate(inputs.checkout_landings[0].downsell_section),
   ...
   const handleCreateUpsell = () => {
      ...
      // if (!inputs.offer_id || !inputs.pricing_id || !inputs.headline.length) {
      //    if (isPrint('Please fill required fields.')) {
   ...
   const handleSaveTemplate = (template) => {
      ...
      onChange({
         ...
         checkout_landings: [{
            ...inputs.checkout_landings[0],
            downsell_section: template.sections,
         }],
      });
   };
      ...
      return (
         <AdminContainer className='upsell__edit__showed'>
            <UpsellCheckoutView
               ...
               // } }
               initalTemplate={ { sections: generateEditingTemplate(inputs.checkout_landings[0].downsell_section) } }
               goBack={ () => {
                  setEditingTemplateName(null);
                  QueryParams.setHash('');
               } }
               inputs={ { ...inputs, priceToView, offerView: { name: 'asd' } } }
            />
         </AdminContainer>
      );
   }
               ...
               <AdminContainer.Content>
                  {isCustomSize ? (
                     <UpsellCreateCheckout
                        ...
                        upsell={ { ...inputs, priceToView, offerView: { name: 'asd' } } }
                     />
                  ) : (
                     <UpsellCreateView
